...
- **host 组合行**（`~/.dsh/profiles/web/cordis.patch.yml` 的 `sim-bridge-host`）：
  `lib/index.mjs` 提供 `simBridge` 服务（桥接进程 + JSONL 命令总线 + 世界状态）、
...
| 文件 | 作用 |
|---|---|
| `lib/index.mjs` | host 半（**必须 ESM/.mjs**）：simBridge 服务 + `/sim-bridge/rpc` 路由（inject `subprocess`/`webServer` 硬依赖；`ctx.effect(() => disposer)` 包裹清理） |
...
- **已踩过的坑（勿回退）**：
  - **host 半必须是 ESM `.mjs`，不要退回 CJS `require`**。CJS 下 `require('@deepseek-ai/schemastery')` 会命中其 `index.cjs`，而 `index.cjs:1` 是 `require('@deepseek-ai/cosmokit')`（纯 ESM 包）；cordis 用 `Promise.allSettled` 并发 import 全部插件，装插件改变并发批次后即触发 Node 的 `ERR_REQUIRE_ESM_RACE_CONDITION`，整个 harness 启动直接报错退出。改成 ESM 后走 schemastery `index.mjs`（纯 `import`），无竞态。证据：禁用 `sim-bridge-host` 即可正常启动。`.mjs` 后缀确保不依赖包级 `"type"`，`tools.js`/`client.js` 保持 CJS 不受影响。
