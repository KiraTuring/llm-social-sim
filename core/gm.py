...
class GMAgent:
    ...
    def _build_gm_prompt(self) -> str:
        ...
        lines.append("- 不要有冗余信息，如果调用多个工具，各工具给出的信息应当是互补的，不在一个工具中给出事件的全部信息，从而让角色自己去拼凑事件的全貌。")
        lines.append("比如，如果事件是\"引擎室的冷却系统效率下降\"，可以用 modify_environment 工具修改冷却效率，在 narrate 工具中只告诉角色\"你听到引擎室传来一阵异响\"，而不是直接告诉角色\"冷却效率下降了\"。")
        lines.append("")
