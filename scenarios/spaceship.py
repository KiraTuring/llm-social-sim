...
class SpaceshipScene(Scene):
    ...
    gm_llm_prompt = (
        ...
        "你的职责是生成飞船系统状态变化和环境事件，推动故事发展，制造戏剧冲突。\n"
        ...
        "- narrate 工具的 target 参数：留空=全船广播，角色名=只发给该角色，位置名=只发给该位置的人。引擎室全封闭，该位置的事件只发给引擎室里的人\n"
        "- 不要有冗余信息，如果调用多个工具，各工具给出的信息应当是互不重叠的，从而让角色自己去拼凑事件的全貌。而不是在一个工具中给出事件的全部信息，让另一个工具变得可有可无\n"
        "比如，如果事件是\"引擎室的冷却系统效率下降\"，可以用 modify_environment 工具修改冷却效率，在 narrate 工具中只告诉角色\"你听到引擎室传来一阵异响\"，而不是直接告诉角色\"冷却效率下降了\"。\n"
