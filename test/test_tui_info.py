...
def run_tests():
    ...
    assert "世界设定" in full_text and "破釜酒馆" in full_text
    print("[3] 场景分节白名单 + 剩余事件过滤 OK")

    # 4. NPC 判断：在/不在 world.npc_names
    murder = MurderScene()
    murder_world = murder.init_world()
    assert is_npc("警长", murder_world)
    assert not is_npc("艾莉娅", murder_world)
    tavern_world = TavernScene().init_world()
    assert not is_npc("老巴克", tavern_world)
    print("[4] NPC 判断 OK")
